...
var ws = new WebSocket('wss://' + location.host + '/mixing-websocket');
var video;
var webcam;
var receive;
var screenPeer;
    ...
    if (!screenPeer) {
        ...
        var options = {
            ...
            mediaConstraints: {
                ...
                video: {
                    frameRate: {
                        min: 12, ideal: 18, max: 24
                    },
                    width: {
                        min: 540, ideal: 600, max: 720
                    },
                    height: {
                        min: 320, ideal: 600, max: 640
                    }
                }
            },
...
function webcamOnOfferViewer(error, offerSdp) {
    if (error)
        return console.error('Error generating the offer');
    console.info('Invoking SDP offer callback function ' + location.host);
    var message = {
...
function screenOnOfferViewer(error, offerSdp) {
    if (error)
        return console.error('Error generating the offer');
    console.info('Invoking SDP offer callback function ' + location.host);
    var message = {
...
function dispose() {
    ...
    hideSpinner(video, webcam, receive);
