 ...
 * @author 木数难数
 */
@ServerEndpoint(value = "/mixing-websocket")
public class MixingWebSocket {

    private KurentoClient kurento = KurentoClient.create("ws://192.168.1.180:8888/kurento");
    private final Logger log = LoggerFactory.getLogger(MixingWebSocket.class);
    ...
    private synchronized void viewer(final Session session, JsonObject jsonMessage, String type)
        ...
        if (SCREEN.equals(type)) {
            ...
            screenPresenterUserSession.getWebRtcEndpoint().connect(hubPort1);

            HubPort hubPort2 = new HubPort.Builder(composite).build();
            webcamPresenterUserSession.getWebRtcEndpoint().connect(hubPort2);
